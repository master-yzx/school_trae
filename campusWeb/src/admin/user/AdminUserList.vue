<template>
  <div class="page">
    <section class="section">
      <div class="toolbar">
        <el-button type="primary" @click="openEdit(null)">新增用户</el-button>
        <el-select
          v-model="status"
          placeholder="状态"
          class="w-140"
    ...
    <el-dialog v-model="editVisible" :title="editForm.id ? '编辑用户' : '新增用户'" width="420px">
...
async function save() {
  await request.post('/admin/users/save', editForm.value);
...
function batchDisable() {
  ...
  request.post('/admin/users/batch/disable', { ids: selectedIds.value }).then(() => {
...
function batchEnable() {
  ...
  request.post('/admin/users/batch/enable', { ids: selectedIds.value }).then(() => {
